...
export default function Sales({ sales, facets }) {
  ...
  const [visiblePages, setVisiblePages] = useState(10); // Number of visible page buttons
      ...
      <div className="content">
        ...
        <div className="pagination">
          ...
          {Array.from({ length: totalPages }, (_, index) => index + 1).map((pageNumber) => (
            (pageNumber >= startPage && pageNumber < startPage + visiblePages) || // Display visible pages
              (pageNumber === totalPages && totalPages <= startPage + visiblePages - 1) ? ( // Display the last page when it's within the visible range
              <button
                key={pageNumber}
            ...
            ) : null // Hide other pages
          ))}
          <button onClick={handleNext} disabled={startPage + visiblePages - 1 >= totalPages}>
            Next
          </button>
        </div>
      </div>
